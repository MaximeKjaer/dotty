...
class Definitions {
  ...
  def AnyValType = AnyValClass.typeRef
    ...
    lazy val Any_==       = enterMethod(AnyClass, nme.EQ, methOfAny(BooleanType), Final | Stable)
    ...
    lazy val Any_isInstanceOf = enterT1ParameterlessMethod(AnyClass, nme.isInstanceOf_, _ => BooleanType, Final | Stable)
  ...
  lazy val AssumePureAnnotType = ctx.requiredClassRef("scala.annotation.assumePure")
  def AssumePureAnnot(implicit ctx: Context) = AssumePureAnnotType.symbol.asClass
  ...
  lazy val ExtractAnnotType = ctx.requiredClassRef("scala.annotation.extract")
  def ExtractAnnot(implicit ctx: Context) = ExtractAnnotType.symbol.asClass
